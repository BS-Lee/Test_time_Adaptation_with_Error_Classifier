...
class SSEDC():
    ...
    def test_err_adapt(self, x, y, supervised):
        ...
        if(err):self.err_num+=1
        if(len(self.sampleBuf[err][predict])>15):self.sampleBuf[err][predict].pop(0)

        return self.err_num    #return saved num
    
    def continue_train_sample(self, buffer):
        self.net.train()
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.SGD(self.net.parameters(), lr=0.00001,
...
if __name__=="__main__":
    ...
    for corroption in corroptions:
        ...
        for i in [1]:
            ...
            semiSuperviseInterval=1
            timing=0
            score_history=[]
            train_data_buffer=[]
            for (x,y) in zip(x_train,y_train):
                dataBuffer[1]+=1    #출력용 test set 시작위치 설정
                if(dataBuffer[1]-dataBuffer[0]>100):dataBuffer[0]+=1    #buffer length is 100
                supervised=0
                timing+=1
                #semi supervise 타이밍을 계산
                if(timing==semiSuperviseInterval):
                    timing=0
                    supervised=1
                #error인지 test하고 분류기에 샘플을 수집하여 adapt.
                #이후 반환값으로 에러 샘플 개수를 받아서,
                #에러 샘플이 충분히 확보되면 분류기가 동작한다고 가정하여
                #base model의 test-domain 학습을 진행함.
                nx=x.unsqueeze(0)
                err_num=ssedc.test_err_adapt(nx,y,supervised=supervised)

                #test-domain learning                
                basemodelOutput =net(nx).cpu().detach().numpy()
                predict = np.argmax(basemodelOutput)
                modelTruth = predict==y
                ssedcResponse = ssedc.test_err(predict,basemodelOutput)
                if(err_num>20 and (ssedcResponse)):
                    print("learning operation")
                    train_data_buffer.append([nx,y])
                    if(len(train_data_buffer)>20):train_data_buffer.pop(0)
                    ssedc.continue_train_sample(train_data_buffer)
